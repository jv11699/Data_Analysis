'''
 A file that grabs the data from the website of the university at buffalo volleyball team  base on the year
...
soup = BeautifulSoup(url_source, 'lxml')  # Conversion to a beautiful soup object
links = [a for a in (td.find('a', href=True) for td in soup.findAll('td')) if a]  # Grabs all the a tags which contains the links
stringLink = []  # Should contain of all the links for the link!
for link in links:
    if year == 2017:
        stringLink.append('http://ubbulls.com/sports/wvball/2017-18/files/' + link['href'])  # Each link in the 2017 website is appended inside the StringLink array
    else:
        stringLink.append('http://ubbulls.com/sports/wvball/2018-19/files/' + link['href'])  # Each link in the 2018 website is appended inside the StringLink array

def convBStoString(bs_objectArr):
    '''
...
def match(match_number, mode='score'):
    ...
    **Initializations are inside this function to have a better Big-O complexity**
    ...
    '''
    if match_number > len(stringLink):
    ...
    source_scores = urllib.request.urlopen(stringLink[match_number]).read()  # the url request that is determined through the match number
    ...
    if mode == 'score':
        ...
        scorePattern = re.findall(r'\d{1,2}-\d{1,2}', convBStoString(scores))  # using regular expressions-it finds pattern of [digits - digits]
        return scorePattern
    ...
    # returns the details from the game or which team is serving.
        ...
        if year == 2018: #This is added due to different implementation in the html table in the link of 2017 and 2018
    ...
    #should return the name of that match
        ...
        match_name = [td for td in(soup.find_all('td', {'align': 'left'}))]  # list of all the match names of that year
...
def grabTeamScore(team, match_number, mode = 'score'):
    '''
    ...
    if mode == 'score':
        detail = match(match_number, 'detail')[0]
        ...
        if re.search(r'Point UB', detail) and re.search(r'1-0', scoresarr[0]) \
           or re.search(r'Point UB', detail) is None and re.search(r'0-1', scoresarr[0]):
                ub_pattern,opp_pattern  = r'\d{1,2}(?=-)', r'(?<=-)\d{1,2}'

        else:
                #This is added due to the different implementations between websites
                ub_pattern,opp_pattern = r'(?<=-)\d{1,2}',r'\d{1,2}(?=-)'
    ...
    #the statistics of each player are returned
        ...
        data_frame = match(match_number, 'data frame')
...
#print(match(match_number = 9, mode = 'score')[48])
...
#print(lengthOfScores-1)
#print(grabTeamScore(team = 'ub',match_number = 9,mode = 'score')[48])
#print(re.search(r'0-1|1-0',match(match_number=9,mode='score')[48]))